 ...
 */
function CHtmlEditorView(oOptions)
{
	this.oOptions = _.extend({
		...
		HiddenUploadParameters: {},
		CAttachmentModel: null
	}, oOptions);
	this.oParent = null;
	...
	this.bInsertImageAsBase64 = this.oOptions.InsertImageAsBase64;
	this.bAllowFileUpload = !(this.bInsertImageAsBase64 && window.File === undefined);
	this.bAllowInsertImage = this.oOptions.AllowInsertImage;
	...
	this.sDefaultFont = this.oOptions.DefaultFontName;
	...
	this.iDefaultSize = this.oOptions.DefaultFontSize;
	...
	this.bAllowChangeInputDirection = this.oOptions.AllowChangeInputDirection;
...
{
	if (!this.inactive() && this.oOptions.AllowInsertImage && !this.visibleInsertImagePopup())
	{
		oEvent.stopPropagation();
...
{
	if (this.oOptions.AllowInsertImage && this.imagePathFromWeb().length > 0)
	{
...
{
	if (this.oOptions.CAttachmentModel)
	{
		var
			oAttachment = new this.oOptions.CAttachmentModel(),
			sViewLink = '',
			bResult = false
		;
		...
		if (this.oOptions.AllowInsertImage && sViewLink.length > 0)
		{
...
CHtmlEditorView.prototype.initUploader = function ()
{
	if (this.imageUploaderButton() && !this.oJua)
	{
		this.oJua = new CJua({
			...
			'hiddenElementsPosition': this.oOptions.IsRTL ? 'right' : 'left',
			'disableMultiple': true,
			'disableAjaxUpload': false,
			'disableDragAndDrop': true,
			'hidden': this.oOptions.HiddenUploadParameters
		});
...
{
	if (this.oOptions.AllowInsertImage && this.uploaderAreaDom() && !this.editorUploader)
	{
		var
			fBodyDragEnter = null,
			fBodyDragOver = null
...
CHtmlEditorView.prototype.onEditorDrop = function (sUid, oData) {
	...
	{
		if (this.oOptions.AllowInsertImage && 0 === oData.File.type.indexOf('image/'))
		{
			oFile = oData.File;
			if (this.oOptions.ImageUploadSizeLimit > 0 && oFile.size > this.oOptions.ImageUploadSizeLimit)
			{
